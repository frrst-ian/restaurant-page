const menuPage = () => {
    ...
    const createDishTitle = (titleText, dishDescription) => {
        const div = document.createElement('div');
        div.classList.add('maindish-div');
        const title = document.createElement('p');
        title.textContent = titleText;
        title.id = 'dish-title';
        div.appendChild(title);
        const dishDesc = document.createElement('p');
        dishDesc.id = 'dish-desc';
        dishDesc.textContent = dishDescription;
        div.appendChild(dishDesc);
        return div;
    }

    const div1 = createDishTitle('Sushi', 'A staple rice dish of Japanese cuisine, consisting of cooked rice'
        + ' flavoured with vinegar and a variety of vegetable, egg, or raw seafood garnishes and served cold');
    const div2 = createDishTitle('Ramen', 'The fish, which must be utterly fresh, is sliced paper thin'
        + ' or alternately one-quarter to one-half inch (0.75–1.5 cm) thick, cubed, or cut in strips, according to the nature of the fish.');
    const div3 = createDishTitle('Sashimi', ' Japanese noodle soup, with a combination of a rich flavoured broth, one of a variety of'
        + ' types of noodle and a selection of meats or vegetables, often topped with a boiled egg.');

    const menuText = document.createElement('p');
    menuText.textContent = 'More coming soon..';
    ...
    document.getElementById('content').appendChild(menuSvg);
};
